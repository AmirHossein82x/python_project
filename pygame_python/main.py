...
def draw_window(red, yellow, red_bullets, yellow_bullets, red_health, yellow_health):
     ...
     red_health_text = HEALTH_FONT.render('Health: ' + str(red_health), 1, WHITE)  # we put 1 always 
     yellow_health_text = HEALTH_FONT.render('Health: ' + str(yellow_health), 1, WHITE)  # we put 1 always 
...
def handle_bullets(yellow_bullet, red_bullet, yellow, red):
    for bullet in yellow_bullet:
        bullet.x += BULLET_VEL
        if red.colliderect(bullet):  # to understand that if 2 object hit together or not
    ...
    for bullet in red_bullet:
        bullet.x -= BULLET_VEL
        if yellow.colliderect(bullet):  # to understand that if 2 object hit together or not
